 ...
 */
PanaAWHEClient::PanaAWHEClient(){}
PanaAWHEClient::PanaAWHEClient(const IPAddress ip){
	begin(ip);
}
 ...
 */
void PanaAWHEClient::begin(const IPAddress ip){
 ...
 */ 
void PanaAWHEClient::serialOutput(bool flag){
	_serialOutput = flag;
}

void PanaAWHEClient::runLoop() {
 ...
 * 
 */
void PanaAWHEClient::connect() {
}

bool PanaAWHEClient::doPan(uint8_t panSpeed) {	// 01-99, 50 is neutral
	if (isReady())	{
...
bool PanaAWHEClient::doTilt(uint8_t tiltSpeed) {	// 01-99, 50 is neutral
	if (isReady())	{
...
bool PanaAWHEClient::doZoom(uint8_t zoomSpeed) {	// 01-99, 50 is neutral
	if (isReady())	{
...
bool PanaAWHEClient::doPanTilt(uint8_t panSpeed,uint8_t tiltSpeed) {	// 01-99, 50 is neutral
	if (isReady())	{
...
bool PanaAWHEClient::setAutoFocus(bool enable) {
	if (isReady())	{
...
bool PanaAWHEClient::doFocus(uint8_t focusPos) {	// 01-99, >50 = Far
	if (isReady())	{
...
bool PanaAWHEClient::onTouchAutofocus() {
	if (isReady())	{
...
bool PanaAWHEClient::setAutoIris(bool enable) {
	if (isReady())	{
...
bool PanaAWHEClient::deletePreset(uint8_t presetNum) {	// 00-99
	if (isReady())	{
...
bool PanaAWHEClient::storePreset(uint8_t presetNum) {	// 00-99
	if (isReady())	{
...
bool PanaAWHEClient::recallPreset(uint8_t presetNum) {	// 00-99
	if (isReady())	{
...
bool PanaAWHEClient::power(bool enable) {
	if (isReady())	{
...
bool PanaAWHEClient::isReady()	{
	return !_activeHTTPRequest;
}
void PanaAWHEClient::_sendPtzRequest(const String command) {
...
void PanaAWHEClient::_sendCamRequest(const String command) {
